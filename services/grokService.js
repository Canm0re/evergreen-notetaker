...
import { EXTRACT_CONCEPTS_PROMPT, GENERATE_NOTE_PROMPT, INTERLINK_NOTES_PROMPT } from '../constants.js';
...
const safeParseJsonResponse = (responseText) => {
  try {
...
const generateSingleNoteBody = async (bookContent, title) => {
  const messages = [
...
const interlinkNotes = async (notes) => {
  ...
  const response = await callGrokWithRetries(messages);
  const linkedNotes = safeParseJsonResponse(response);
...
export const processBookWithGrok = async (initialState, onProgress) => {
